...
class ReplayVersion6Offsets(ReplayOffsets):
    ...
    def extract_spy_username(self, bytes):
        ...
        return self._read_bytes(bytes, 0x64, spy_username_length).decode()

    def extract_sniper_username(self, bytes):
        spy_username_length = bytes[0x2E]
        sniper_username_length = bytes[0x2F]
        return self._read_bytes(bytes, 0x64 + spy_username_length, sniper_username_length).decode()
    ...
    def extract_spy_display_name(self, bytes):
        ...
        if spy_displayname_length == 0:
            ...
            return self._read_bytes(bytes, 0x64 + spy_username_length + sniper_username_length, spy_displayname_length).decode()
    ...
    def extract_sniper_display_name(self, bytes):
        ...
        if sniper_displayname_length == 0:
            ...
            return self._read_bytes(bytes, 0x64 + spy_username_length + sniper_username_length + spy_displayname_length, sniper_displayname_length).decode()
